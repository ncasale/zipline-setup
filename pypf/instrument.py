...
class DataframeInstrument(Instrument):
    ''' Security object that uses a dataframe as its source '''
    def __init__(self, symbol, dataframe, force_download=True, force_cache=False,
                 period=10, debug=False, data_directory='~/.pypf/data'):
        """Initialize the security."""
        super().__init__(symbol, force_download, force_cache,
                         period, debug, data_directory)
    ...
    def _download_data(self):
        self._dataframe.to_csv(self._data_path)
        return True
